...
```bash
git clone https://github.com/SteveVaneeckhout/Our.Umbraco.ContentDashboard.git
cd Our.Umbraco.ContentDashboard
...
Open the **Content** section: the *My Content*, *Scheduled* and *Ownership* tabs are the package.

## The demo content

The demo content is a theme-park site, "Coaster World": 67 documents, all created by the
unattended admin user, so the *My Content* tab has something in it immediately. Two deliberate
future schedules keep the *Scheduled* tab non-empty — **Acrobat** publishes on 2026-12-01 and
**Alton Towers** unpublishes on 2026-11-15. Clear them from each page's publish flyout when they
stop being useful.
...
```bash
dotnet build Our.Umbraco.ContentDashboard.slnx
# (no test project yet)
...
fork, branch, make sure `dotnet build`, `dotnet format --verify-no-changes` all pass,
