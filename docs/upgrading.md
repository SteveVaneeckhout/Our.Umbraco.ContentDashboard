...
> every fixture `editorUiAlias` exists in 17.7.0 (18.2.0 only adds `Umb.PropertyEditorUi.ElementPicker`),
> and the client type-checks against `@umbraco-cms/backoffice@17.7.0` unchanged.
...
`/umbraco/openapi/<apiname>.json`. Umbraco 17's Swashbuckle filter does not have it - `TransferAll`
declares a `403` and `/umbraco/swagger/contentdashboard/swagger.json` generates fine on 17.7.0. The
swagger probe in the verify script still runs first, because it is still the cheapest way to catch a
controller that no longer binds or DI that no longer resolves.
